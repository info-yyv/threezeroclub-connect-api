    ...
    try {
        ...
        promiseImages.then(async (imagesUrl) => {
            promiseVideos.then(async (videosUrl) => {
                ...
                //all user message broadcast test
                conversation.users.forEach((user) => {
                    socket.emit('send-message', { userid: user._id, conversation: filterConversationMessages(updatedConversation, user._id) })
                })
                //all user message broadcast test ends
...
exports.createGroupConversation = async (req, res) => {
    try {
        let destinationUsers = req.body.destination
        
        let pseudosData = [] 
        let pseudoName = req.body.group_name
        
        destinationUsers.push(req.user._id)

        destinationUsers.forEach((element_user) => {
            pseudosData.push(
                {
                    userid: element_user,
                    content: pseudoName
