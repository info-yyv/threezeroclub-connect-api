...
const io = require("socket.io")(server,{
    pingTimeout: 60000,
    cors: {
      origin: ["http://connect.3zero.club", "http://localhost:4200"],
      // credentials: true,
    },
});
...
server.listen(process.env.PORT || 5000, () => {
    io.on('connection', (socket) => {
        ...
        socket.on("stop-typing", (data) => {
            // socket.on("stop-typing", (roomId) => socket.in(roomId).emit("stopped-typing"));
            const userindex = ConnectedUsers.findIndex(connecteduser => {
                return connecteduser.userid === data.userid
            })
            ...
            if (userindex >= 0)
                ConnectedUsers[userindex].socketIds.forEach(socketId => {
                    socket.broadcast.to(socketId).emit('stop-typing', data)
                })
        });
